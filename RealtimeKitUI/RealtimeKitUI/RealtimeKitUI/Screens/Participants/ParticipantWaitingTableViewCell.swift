...
class BaseParticipantWaitingTableViewCell: ParticipantTableViewCell {
    let crossButton = {
        let button = RtkButton(style: .iconOnly(icon: RtkImage(image: ImageProvider.image(named: "icon_cross"))), dyteButtonState: .active)
    ...
    let tickButton = {
        let button = RtkButton(style: .iconOnly(icon: RtkImage(image: ImageProvider.image(named: "icon_tick"))), dyteButtonState: .active)
