...
extension WebinarParticipantViewController: UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let cell = cell as? ParticipantInCallTableViewCell {
            ...
            cell.buttonCrossClick = { [weak self] button in
                guard let self = self else {return}
                button.showActivityIndicator()
                self.viewModel.mobileClient.stage.denyAccess(userIds: [model.participant.userId])
            ...
            cell.buttonTickClick = { [weak self] button in
                guard let self = self else {return}
                button.showActivityIndicator()
                self.viewModel.mobileClient.stage.grantAccess(userIds: [model.participant.userId])
                button.hideActivityIndicator()
                self.reloadScreen()
            }
    ...
    private func createMoreMenuForViewers(participantListner: RtkParticipantUpdateEventListner, indexPath: IndexPath)-> Bool {
        var menus = [MenuType]()
        ...
        let moreMenu = RtkMoreMenu(title: participant.name, features: menus, onSelect: { [weak self] menuType in
            ...
            case .allowToJoinStage:
                self.viewModel.mobileClient.stage.grantAccess(userIds: [participant.userId])
    ...
    private func createMoreMenu(participantListner: RtkParticipantUpdateEventListner, indexPath: IndexPath)-> Bool {
        var menus = [MenuType]()
        let participant = participantListner.participant
        let hostPermission = self.viewModel.mobileClient.localUser.permissions.host
        ...
        let moreMenu = RtkMoreMenu(title: participant.name, features: menus, onSelect: { [weak self] menuType in
            ...
            case .removeFromStage:
                self.viewModel.mobileClient.stage.kick(userIds: [participant.userId])
            case .kick:
                if let remoteParticipant = participant as? RtkRemoteParticipant {
                    remoteParticipant.kick()
                }
