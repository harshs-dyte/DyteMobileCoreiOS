...
public class RtkChatViewController: RtkBaseViewController, NSTextStorageDelegate, SetTopbar {
    ...
    fileprivate var messages: [RtkChatMessage]?
    ...
    let sendFileButton = RtkButton(style: .iconOnly(icon: RtkImage(image: ImageProvider.image(named: "icon_chat_add"))), dyteButtonState: .focus)
    let sendImageButton = RtkButton(style: .iconOnly(icon: RtkImage(image: ImageProvider.image(named: "icon_image"))), dyteButtonState: .active)
    let sendMessageButton = RtkButton(style: .iconOnly(icon: RtkImage(image: ImageProvider.image(named: "icon_chat_send"))), dyteButtonState: .active)
    ...
    let meetingObserver: RtkMeetingEventListner
    ...
    override public init(meeting: RealtimeKitClient) {
        meetingObserver = RtkMeetingEventListner(mobileClient: meeting)
    ...
    private func handleRateLimitError(error: ChatTextError?) -> Bool {
        if error?.code == ChatTextErrorCode.ratelimitbreached {
...
extension RtkChatViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        {
            if let cell = tableView.dequeueReusableCell(withIdentifier: "FileMessageCell", for: indexPath) as? FileMessageCell, let msg = messages?[indexPath.row] as? RtkFileMessage {
            ...
            if let cell = tableView.dequeueReusableCell(withIdentifier: "MessageCell", for: indexPath) as? MessageCell, let msg = messages?[indexPath.row] as? RtkImageMessage {
                if let fileURL = URL(string: msg.link) {
...
extension RtkChatViewController: RtkChatEventsListener {
    ...
    public func onNewChatMessage(message: RtkChatMessage) {
        notificationBadge.isHidden = true
    ...
    public  func onChatUpdates(messages: [RtkChatMessage]) {
        if isOnScreen {
