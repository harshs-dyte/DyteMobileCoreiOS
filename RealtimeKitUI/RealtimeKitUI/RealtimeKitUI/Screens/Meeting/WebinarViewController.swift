...
public class WebinarViewController: MeetingViewController {
    ...
    public override func refreshMeetingGrid(forRotation: Bool = false) {
        ...
        if (mediaPermission.audioPermission == RtkMediaPermission.allowed || mediaPermission.video.permission == RtkMediaPermission.allowed) && meeting.participants.active.isEmpty && StageStatus.getStageStatus(mobileClient: meeting) == .canJoinStage {
