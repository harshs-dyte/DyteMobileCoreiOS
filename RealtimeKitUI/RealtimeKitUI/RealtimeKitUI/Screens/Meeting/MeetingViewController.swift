...
extension MeetingViewController: RtkLivestreamEventListener {
    ...
    public func onLivestreamStateChanged(oldState: RealtimeKitCore.LivestreamState, newState: RealtimeKitCore.LivestreamState) {
...
extension MeetingViewController: RtkChatEventsListener {
    ...
    public func onNewChatMessage(message: RtkChatMessage) {
        if message.userId != meeting.localUser.userId {
            var chat = ""
            if  let textMessage = message as? RtkTextMessage {
